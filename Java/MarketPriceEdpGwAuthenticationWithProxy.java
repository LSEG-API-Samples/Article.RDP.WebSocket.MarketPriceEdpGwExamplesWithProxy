 ...
 */
public class MarketPriceEdpGwAuthenticationWithProxy {
	...
	public static void main(String[] args) {
        ...
        int _proxyPortInt;
        try{
        	_proxyPortInt = Integer.parseInt(proxyPort);
        }catch(NumberFormatException ex){
        	_proxyPortInt = 0;
        	System.out.println("NumberFormatException: --proxy_port is not a number, not using proxy");
        }
        if(!proxyHost.isEmpty() && _proxyPortInt!=0){
        	proxy = new HttpHost(proxyHost, _proxyPortInt, "http");
	...
	public static WebSocket connect() throws IOException, WebSocketException
    {
		...
		if(config!=null)
			factory.getProxySettings().setServer("http://"+proxyHost+":"+proxyPort);		
		return factory.createSocket(server)
